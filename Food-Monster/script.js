...
function injectHTML(list) {
  ...
  target.innerHTML = "";
  list.forEach((item) => {
    const capitalizedRestaurantName = capitalizeEveryWord(item.name);
    const str = `<li>✨${capitalizedRestaurantName}✨</li>`;
...
async function mainEvent() {
  ...
  if (arrayFromJson.length > 0) {
    // these functions run once the data has loaded
    ...
    form.addEventListener("submit", async (submitEvent) => {
      ...
      */
      const restaurants = currentArray.filter((item) =>
        Boolean(item.geocoded_column_1)
      );
